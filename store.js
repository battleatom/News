...
export class Store{
  ...
  suppressionKey(){return ["D","NR","NW"].map(reason=>`${reason}:`+(this.suppressed.get(reason)||[]).map(r=>r?.id||r?.url_key||r?.urlKey||r?.url||`${r?.title_key||r?.title||""}|${r?.source_key||r?.source||""}|${r?.category||r?.tab||""}`).join("~")).join(";")}\n  allStories(){const key=this.suppressionKey();if(this._allStoriesFeed===this.feed&&this._allStoriesSuppression===key)return this._allStoriesCache;const rows=Object.keys(this.feed?.categories||{}).flatMap(category=>this.categoryStories(category));this._allStoriesFeed=this.feed;this._allStoriesSuppression=key;this._allStoriesCache=rows;return rows}
  ...
  storyById(id){return this._storyIndex.get(id)||this.bookmarks.find(row=>row.id===id)||null}
}
