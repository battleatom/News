...
cluster=load('cluster',Path('scripts/cluster_related_coverage.py'))
why=load('why',Path('scripts/update_why_matters.py'))
...
def item(cat,title,source,desc,link):
 ...
 return x
# Same event, different wording: payment amount is a strong fingerprint.
...
assert cluster.same_event(a,b)
assert not cluster.same_event(a,c)
# Commemoration/date matching is generic event logic, not a 9/11 title filter.
...
# Why It Matters must use the final paraphrased description, not only headline words.
...
# Frontend location patch must not contain the old news-card fallback constructors.
loc=(ROOT/'scripts/patch_legislation_location.py').read_text()
assert 'coverageItem(' not in loc and 'stateCoverage(' not in loc
assert 'Only official legislative records appear as cards' in loc
full=(ROOT/'scripts/patch_full_why_matters.py').read_text()
assert '-webkit-line-clamp:unset' in full and 'overflow:visible' in full
