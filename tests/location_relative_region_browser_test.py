...
def region_signature(browser, state, city):
    context=browser.new_context(viewport={'width':1280,'height':900})
    context.add_init_script(f"""
      localStorage.setItem('underreported-state','{state}');
      localStorage.setItem('underreported-location','{city}');
    ...
    assert data, f'No regional stories for {state}/{city}'
    context.close()
    return data
...
def main():
    with sync_playwright() as p:
        ...
        nm=region_signature(browser,'NM','Farmington, New Mexico')
        ma=region_signature(browser,'MA','Boston, Massachusetts')
        browser.close()
    ...
    print(f'LOCATION-RELATIVE REGION BROWSER TEST PASSED: NM={len(nm)} Northeast={len(ma)} overlap={overlap}')
