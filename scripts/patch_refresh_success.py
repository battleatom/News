...
P = Path('index.html')
MARKER = 'refresh-success-handling-v3'
...
replacement = r'''async function fetchNewsDocument(){
  ...
  // Last-resort public-repo fallback. This avoids false "RSS failed" states when
  // GitHub Pages/CDN briefly serves a stale or unavailable project-site asset.
  addUrl('https://raw.githubusercontent.com/battleatom/News/main/News');
  let lastError=null;
...
async function refreshNewsFromPage(manual=false){
  ...
  try{
    ...
    try{
      ...
      if(status)status.textContent='Feed downloaded · display retry scheduled';
      pullStatusHtml(allItems.length,false,'Display refresh retry scheduled');
      return false;
    }
    ...
    console.warn('News feed refresh temporarily unavailable after retries:',e);
    nextScheduledPull=Date.now()+60000;
    window.nextScheduledPull=nextScheduledPull;
    if(allItems.length){
      ...
      if(status)status.textContent='Feed retry scheduled · showing '+allItems.length+' current stories';
      pullStatusHtml(allItems.length,false,'Temporary feed delay · current stories kept');
    }else{
      // Do not report a backend RSS failure from a transient browser/CDN fetch.
      // Keep the UI neutral and automatically retry in one minute.
      if(status)status.textContent='Feed temporarily unavailable · retrying';
      pullStatusHtml(0,false,'Temporary feed delay · retry scheduled');
    }
    return false;
  }finally{
...
print('Applied resilient feed fallbacks and neutral transient-failure status handling.')
