...
new="""let d=null,liveError=null;
   ...
   if(!d?.events?.length){const r=await fetch('nfl-scoreboard.json?ts='+Date.now(),{cache:'no-store'});if(!r.ok)throw liveError||new Error('cached scoreboard HTTP '+r.status);d=await r.json();}
