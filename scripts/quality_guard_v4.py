...
US_CODES={'al','ak','az','ar','ca','co','ct','de','fl','ga','hi','id','il','in','ia','ks','ky','la','me','md','ma','mi','mn','ms','mo','mt','ne','nv','nh','nj','nm','ny','nc','nd','oh','ok','or','pa','ri','sc','sd','tn','tx','ut','vt','va','wa','wv','wi','wy'}
...
def norm_word(w):
    w=w.lower()
    aliases={'intelligence':'intel','warnings':'warning','warned':'warning','warns':'warning','strikes':'strike','attacks':'attack','elections':'election','lawsuits':'lawsuit'}
...
def tokens(s): return {norm_word(w) for w in re.findall(r'[a-z0-9]+',s.lower()) if len(w)>=3 and w not in STOP}
def entities(s):
    out=set()
...
def same_event(a,b):
    ...
    sh=ta&tb; small=min(len(ta),len(tb)); jac=len(sh)/max(1,len(ta|tb)); ea,eb=entities(a),entities(b)
    acr={w for w in sh if w.isalpha() and len(w)<=4}
    return (len(sh)>=3 and (jac>=.30 or len(sh)/small>=.55)) or (bool(ea&eb) and len(sh)>=2 and len(sh)/small>=.40) or (len(sh)>=3 and bool(acr))

def is_us_domestic(item):
    s=(txt(item,'state') or txt(item,'marketState')).lower()
    return s in US_STATES or s in US_CODES

def preserve_rss(item):
    d=txt(item,'description')
    if len(d)>=70 and 'publisher did not expose enough public article text' not in d.lower() and not txt(item,'rssDescription'): setv(item,'rssDescription',d)
...
def reroute(items):
    ...
    for i in items:
        ...
        elif cat=='federal' and any(k in blob for k in ('canada','canadian','ottawa','canadian federal')) and not any(k in blob for k in ('u.s.','united states','congress','white house')): setv(i,'category','world'); moved+=1
        elif cat=='technology' and any(k in blob for k in ('skyrim','playstation','xbox','nintendo','video game','gaming console')): setv(i,'category','gaming'); moved+=1
    return moved
...
def clean_related(items):
    ...
    for i in items:
        ...
        for tag in ('relatedArticles','xRelated'):
            ...
            for a in list(rel):
                if not same_event(pt,txt(a,'title')): rel.remove(a); removed+=1
    return removed

def x_valid(topic,item):
    blob=(txt(item,'title')+' '+txt(item,'description')).lower()
...
def make_x(src,topic):
    ...
    setv(n,'guid',hashlib.sha1((txt(src,'link')+'|x|'+topic).encode()).hexdigest()); return n
...
def repair_x(root,items):
    ...
    for old in list(xs):
        ...
        if topic in {'World','Celebrities & Public Figures','Science'} and not x_valid(topic,old):
            ...
            if cand is not None:
                idx=list(channel).index(old); channel.remove(old); channel.insert(idx,make_x(cand,topic)); used.add(txt(cand,'link')); fixed+=1
    return fixed
...
def clean_boxoffice():
    ...
    for m in data.get('movies',[]):
        ...
        if not d:continue
        low=d.lower(); title_terms={w for w in re.findall(r'[a-z0-9]+',t) if len(w)>=4}; movieish=any(k in low for k in (' film ',' movie ',' directed ',' stars ',' starring ',' sequel ',' screenplay ')); overlap=any(w in low for w in title_terms)
        if low.startswith('running is a method of terrestrial locomotion') or (not movieish and not overlap and len(d)>180):m['description']=''; changed+=1
...
def rss_brief(item):
    d=useful_rss(item)
    if not d:return ''
    d=re.sub(r'<[^>]+>',' ',d); d=re.sub(r'\s+',' ',d).strip(); d=re.sub(r'\baccording to\b','based on',d,flags=re.I); d=re.sub(r'\bsaid\b','reported',d,flags=re.I); d=re.sub(r'\bwill\b','is expected to',d,flags=re.I)
...
def final_enhance(items):
    ...
    for i in items:
        if txt(i,'briefSource')=='headline-fallback':
            b=rss_brief(i)
            if b:setv(i,'description',b); setv(i,'briefSource','rss-description'); upgraded+=1
...
def assert_clean(items):
    bad_world=[txt(i,'title') for i in items if txt(i,'category')=='world' and is_us_domestic(i)]; bad_fed=[txt(i,'title') for i in items if txt(i,'category')=='federal' and 'canada' in (txt(i,'title')+' '+txt(i,'description')).lower()]
    if bad_world or bad_fed:raise SystemExit(f'Quality guard failed routing: world={bad_world[:3]} federal={bad_fed[:3]}')

def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--mode',choices=['pre','final'],default='pre'); a=ap.parse_args(); tree=ET.parse(NEWS); root=tree.getroot(); items=list(root.findall('.//item'))
    if a.mode=='pre':
        for i in items:preserve_rss(i)
        moved=reroute(items); related=clean_related(items); xfix=repair_x(root,items); box=clean_boxoffice(); tree.write(NEWS,encoding='utf-8',xml_declaration=True); items=list(root.findall('.//item')); assert_clean(items); print(f'Quality guard pre: rerouted={moved} unrelated-links-removed={related} x-repaired={xfix} boxoffice-cleared={box}')
    else:
        up,why=final_enhance(items); related=clean_related(items); tree.write(NEWS,encoding='utf-8',xml_declaration=True); assert_clean(items); print(f'Quality guard final: rss-brief-upgrades={up} why-matters-fixed={why} unrelated-links-removed={related}')
