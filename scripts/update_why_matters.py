...
The content-brief pipeline writes the original factual paraphrase into <description>.
This step runs immediately afterward, so Why It Matters is derived from that same
brief instead of from an unrelated headline keyword or stale collector template.
"""
...
def why_for(item):
    brief=clean(item.findtext('description')); title=clean(item.findtext('title')); cat=clean(item.findtext('category')).lower()
    text=f'{title} {brief}'.lower()
    # Consequence selection is based on the final paraphrased brief. Rules describe
    # affected domains without taking a position on the underlying event or policy.
    if has(text,'war','airstrike','missile','troops','ceasefire','invasion','attack'):
    ...
    elif has(text,'payment','payments','check','checks','rebate','refund','payout','cash','benefit','benefits'):
    ...
    elif has(text,'recall','outbreak','hospital','medicaid','medicare','health','safety'):
