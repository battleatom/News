...
(function(){
  ...
  function renderLegislationCards(visible,total,count){
    ...
    const intro=document.createElement('div');intro.className='x-issues-intro';intro.textContent='Official federal and detected-state legislative records are shown first when available. Established news coverage can supplement gaps and is labeled as supporting coverage.';sec.appendChild(intro);
    ...
    visible.forEach((item,i)=>{
      ...
      const isCoverage=/supporting coverage/i.test(status); const recordLabel=isCoverage?'News source':'Primary record';
      ...
      ar.innerHTML=`<div class="leg-top"><span class="leg-pill">${esc(jurisdiction)}</span><span class="leg-pill">${esc(status)}</span></div><h3><a href="${esc(link)}" target="_blank" rel="noopener noreferrer">${i+1}. ${esc(title)}</a></h3>${what?`<p class="description">${esc(what)}</p>`:''}<div class="leg-grid"><div class="leg-field"><strong>Who it affects</strong><span>${esc(affected)}</span></div><div class="leg-field"><strong>Effective date</strong><span>${esc(effective)}</span></div><div class="leg-field"><strong>What happens next</strong><span>${esc(next)}</span></div><div class="leg-field"><strong>${recordLabel}</strong><span>${esc(source||(isCoverage?'News coverage':'Official source'))}</span></div>${latest?`<div class="leg-field"><strong>Latest official action</strong><span>${esc(latest)}</span></div>`:''}${sponsor?`<div class="leg-field"><strong>Sponsor</strong><span>${esc(sponsor)}</span></div>`:''}</div>${official?`<div class="leg-official"><a href="${esc(official)}" target="_blank" rel="noopener noreferrer">Open official record ↗</a></div>`:''}${support}<div class="meta"><span>${esc(formatDate(date))}</span>${source?`<span class="source">${esc(source)}</span>`:''}</div>`;
      body.appendChild(ar);
    });
