...
(function(){
  ...
  function renderLegislationCards(visible,total,count){
    ...
    head.innerHTML=`<h2>📜 Laws & Legislation</h2><span class="count">Showing ${count} of ${total} stories</span>`;sec.appendChild(head);
    const intro=document.createElement('div');intro.className='x-issues-intro';intro.textContent='Federal, New Mexico, and important Four Corners bills, laws, executive orders, rules, and ordinances — focused on what changed, who it affects, and what happens next.';sec.appendChild(intro);
    const body=document.createElement('div');body.className='section-body';
    if(!visible.length)body.innerHTML='<div class="empty">No significant legislation updates are available right now.</div>';
    visible.forEach((item,i)=>{
      ...
      const jurisdiction=xtext(item,'jurisdiction')||'Federal'; const status=xtext(item,'status')||'Active development'; const what=xtext(item,'whatItDoes')||xtext(item,'description'); const affected=xtext(item,'whoAffected')||'See reporting for details'; const effective=xtext(item,'effectiveDate')||'Not stated'; const next=xtext(item,'nextStep')||'Watch for the next formal action.'; const official=xtext(item,'officialSource');
      ...
      const support=related.length?`<div class="leg-support"><div class="leg-support-title">Supporting reporting</div>${related.map(r=>{const rt=xtext(r,'title');const rl=xtext(r,'link');const rs=xtext(r,'source');return `<a href="${esc(rl)}" target="_blank" rel="noopener noreferrer">${esc(rt)}${rs?`<small>${esc(rs)}</small>`:''}</a>`}).join('')}</div>`:'';
      ar.innerHTML=`<div class="leg-top"><span class="leg-pill">${esc(jurisdiction)}</span><span class="leg-pill">${esc(status)}</span></div><h3><a href="${esc(link)}" target="_blank" rel="noopener noreferrer">${i+1}. ${esc(title)}</a></h3>${what?`<p class="description">${esc(what)}</p>`:''}<div class="leg-grid"><div class="leg-field"><strong>Who it affects</strong><span>${esc(affected)}</span></div><div class="leg-field"><strong>Effective date</strong><span>${esc(effective)}</span></div><div class="leg-field"><strong>What happens next</strong><span>${esc(next)}</span></div><div class="leg-field"><strong>Reporting source</strong><span>${esc(source||'Source unavailable')}</span></div></div>${official?`<div class="leg-official"><a href="${esc(official)}" target="_blank" rel="noopener noreferrer">Official government source ↗</a></div>`:''}${support}<div class="meta"><span>${esc(formatDate(date))}</span>${source?`<span class="source">${esc(source)}</span>`:''}</div>`;
      body.appendChild(ar);
    });
...
print('Added Laws & Legislation tab with impact fields, official source, supporting reporting, and Load More support.')
