...
.nfl-game-card{position:relative;padding:10px 11px!important;min-height:116px!important;margin:0!important;display:flex;flex-direction:column;justify-content:space-between;gap:7px;border:2px solid var(--nfl-home-accent,#cbd5e1)!important;box-shadow:0 2px 8px rgba(15,23,42,.05)}
...
function renderNfl(){
 ...
 function sortedEvents(events){return [...events].sort((a,b)=>{const ra=eventRank(a),rb=eventRank(b);if(ra!==rb)return ra-rb;const ta=eventTime(a),tb=eventTime(b);return ra===2?tb-ta:ta-tb;});}
 ...
 async function load(){
  try{
   ...
   const events=sortedEvents(d.events),hasLive=events.some(ev=>ev.status?.type?.state==='in'),nextIndex=hasLive?-1:events.findIndex(ev=>ev.status?.type?.state==='pre');
   events.forEach((ev,index)=>{
    ...
    card.innerHTML=`<div class="nfl-card-head">${badge}${logos}</div><div class="nfl-matchup-line">${awayName} at ${homeName}${scoreText}</div><div class="nfl-game-subline"><span class="nfl-status">${esc(when)}</span>${tv?`<span class="nfl-tv">${tv}${watchHtml}</span>`:''}</div>`;
...
print('Added NFL team logos, compact matchup lines, and home-team card accents.')
