...
STYLE='''<style id="nfl-live-style">.nfl-live-note{padding:8px 12px;margin-bottom:10px;border:1px solid var(--ui-line);border-radius:12px;background:rgba(255,255,255,.68);font-size:10px;color:#64748b}.nfl-live-note strong{color:#166534}.nfl-game-card .broadcast{margin-top:8px;font-size:10px;color:#475569}.nfl-game-card .broadcast strong{color:#166534}.nfl-live-dot{display:inline-block;width:7px;height:7px;border-radius:50%;background:#16a34a;margin-right:5px;box-shadow:0 0 0 3px rgba(22,163,74,.12)}</style>'''
...
function renderNflLive(){
 ...
 const note=document.createElement('div');note.className='nfl-live-note';note.innerHTML='<strong>Live data:</strong> scores and game status are pulled directly from ESPN\'s public scoreboard endpoint and refreshed every 30 seconds while this tab is open. Broadcast listings identify where the game is carried; availability of a stream may depend on your TV/streaming subscription.';body.appendChild(note);
 async function load(){
  try{
...
print('Added 30-second live NFL score refresh and broadcast/stream information.')
