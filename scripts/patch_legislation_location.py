...
(function(){
 ...
 function jsonItem(rec){
   const official=(rec.officialSource||rec.indexSource||'').trim();
   ...
   item.appendChild(x(doc,'link',official)); item.appendChild(x(doc,'description',rec.description||rec.title||''));
   ...
   item.appendChild(x(doc,'officialSource',official)); item.appendChild(x(doc,'billNumber',rec.billNumber||''));
 ...
 function locationItems(items){
   ...
   const federal=items.filter(i=>(i.querySelector('category')?.textContent?.trim()||'')==='legislation' && /federal/i.test(i.querySelector('jurisdiction')?.textContent||'') && (i.querySelector('officialSource')?.textContent||'').trim());
   let stateRows=[];
   if(code==='NM'){
     stateRows=items.filter(i=>(i.querySelector('category')?.textContent?.trim()||'')==='legislation' && /new mexico/i.test(i.querySelector('jurisdiction')?.textContent||'') && (i.querySelector('officialSource')?.textContent||'').trim());
   }else{
     stateRows=((stateCache?.states||{})[code]?.bills||[]).map(jsonItem).filter(Boolean);
   }
 ...
 window.__categoryRenderers.legislation=function(items){
   ...
   if(sec){
     ...
     if(intro)intro.textContent=`Federal records come from Congress.gov. ${stateName()} follows your detected location. Only official legislative records appear as cards; related journalism may appear only as supporting links on a matching record.`;
   }
 };
...
print('Legislation location renderer now uses official records only; no news fallback cards.')
