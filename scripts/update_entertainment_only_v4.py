...
# High-profile fallback queries are intentionally broad and current. They are used
# only if the normal Clean collector cannot fill a useful page.
CLEAN_FALLBACK_QUERIES=[
    'Zendaya OR Taylor Swift OR Beyonce OR Rihanna OR Lady Gaga OR Ariana Grande OR Billie Eilish entertainment news',
    'Tom Cruise OR Brad Pitt OR Leonardo DiCaprio OR Margot Robbie OR Jennifer Lawrence OR Scarlett Johansson entertainment news',
    'Pedro Pascal OR Jenna Ortega OR Sydney Sweeney OR Florence Pugh OR Timothee Chalamet OR Tom Holland entertainment news',
    'Drake OR Kendrick Lamar OR The Weeknd OR Harry Styles OR Bad Bunny OR Sabrina Carpenter music entertainment news',
]
...
ENTITY_NOISE={'People','People Com','E News','Variety','Billboard','Deadline','The Hollywood Reporter','Rolling Stone','Entertainment Weekly','New York','Los Angeles','United States','North Carolina','South Korean','Prime Video','Netflix','Toronto Film Festival','White House','Associated Press'}

def _text(item): return f"{item.get('title','')} {item.get('description','')} {item.get('source','')}".lower()
def _title(item): return (item.get('title') or '').strip()
def _title_lower(item): return _title(item).lower()
def _key(item):
...
def _source_key(item): return (item.get('source') or '').strip().lower()
def _source_is_adult(item): return any(t in _source_key(item) for t in ADULT_SOURCE_TOKENS)
def _adult_prominence(item):
    text=_text(item);return max([score for name,score in ADULT_PROMINENCE.items() if name in text] or [0])
def _known_name(item):
    text=_text(item);matches=[name for name in MAJOR_NAMES if name in text];return max(matches,key=len) if matches else ''
def _title_entities(item):
    matches=re.findall(r"\b(?:[A-Z][A-Za-z'’.-]+)(?:\s+(?:[A-Z][A-Za-z'’.-]+)){1,3}\b",_title(item));out=[]
    for raw in matches:
        raw=raw.strip(' -–—:,.')
        if len(raw)<5 or any(noise.lower()==raw.lower() for noise in ENTITY_NOISE):continue
        out.append(raw.lower())
    return set(out)
def build_clean_signals(candidates):
    entity_sources={}
    for item in candidates:
        if _source_is_adult(item):continue
        src=_source_key(item)
        for entity in _title_entities(item):entity_sources.setdefault(entity,set()).add(src)
...
def _dynamic_prominence(item,entity_sources):
    ...
    if _known_name(item):best=max(best,175)
    return best
def _story_coverage(item,candidates):
    sources=set()
    for other in candidates:
        if _source_is_adult(other):continue
        if _key(other)==_key(item):sources.add(_source_key(other));continue
        try:same=v4._same_entertainment_event(item,other)
        except Exception:same=False
        if same:sources.add(_source_key(other))
    n=len(sources)
    return 150 if n>=5 else 120 if n==4 else 90 if n==3 else 55 if n==2 else 0
def _clean_relevant(item,cls,prominence):
    title=_title_lower(item);source=_source_key(item)
    if any(term in title for term in FILLER_TERMS):return False
    has_known=bool(_known_name(item));has_role=any(term in title for term in ENT_ROLE_TERMS);repeated=prominence>=95
    specialist=any(token in source for token in SPECIALIST_CLEAN_SOURCES);has_person=bool(_title_entities(item))
    if any(term in title for term in GENERIC_NONCELEB_TERMS) and not (has_known or has_role or repeated):return False
    if has_known or repeated or has_role:return True
    if specialist and has_person and cls in {'MAJOR','PEOPLE / FAMILY','CAREER','AWARDS','PHILANTHROPY'}:return True
    return False
def classify(item,forced_dirty=False,adult_source=False):
    text=_text(item);title=_title_lower(item)
...
def score_item(item,safety,cls,prominence=0,coverage=0):
    ...
    if safety=='clean':
        base={'MAJOR':620,'PEOPLE / FAMILY':560,'CAREER':535,'AWARDS':510,'PHILANTHROPY':455,'ENTERTAINMENT':330}.get(cls,300)+prominence+coverage
        if any(t in title for t in ('gave birth','gives birth','welcomes','baby','pregnant','pregnancy')):base+=45
        if _known_name(item):base+=35
    else:
        base={'ADULT BUSINESS/LEGAL':1000,'ADULT INDUSTRY':920,'CREATOR':880,'ADULT AWARDS':850,'NUDE / PHOTO SHOOT':760,'MATURE FASHION':700,'GOSSIP':650,'RELATIONSHIPS':625,'GOSSIP / LIFESTYLE':600}.get(cls,600)+_adult_prominence(item)
...
def decorate(item,entity_sources,candidates,forced_dirty=False,adult_source=False):
    ...
    safety,cls=tagged;copy=dict(item)
    if safety=='clean':
        prominence=_dynamic_prominence(copy,entity_sources)
        if not _clean_relevant(copy,cls,prominence):return None
        coverage=_story_coverage(copy,candidates);copy['entertainmentProminence']=prominence;copy['entertainmentCoverage']=coverage
    else:
        prominence=_adult_prominence(copy);coverage=0;copy['entertainmentProminence']=prominence;copy['entertainmentCoverage']=''
    copy['entertainmentSafety']=safety;copy['entertainmentLabel']=cls;copy['entertainmentScore']=score_item(copy,safety,cls,prominence,coverage);copy['entertainmentTier']='under-the-radar' if safety=='dirty' else 'ranked'
...
def _strip_tags(value):return re.sub(r'\s+',' ',html_lib.unescape(re.sub(r'<[^>]+>',' ',value or ''))).strip()
...
def fetch_xbiz_direct(limit=20):
    ...
    try:raw=urlopen(Request(url,headers={'User-Agent':'Mozilla/5.0 (compatible; UnderreportedV4/1.0)'}),timeout=20).read().decode('utf-8','ignore')
    except Exception as exc:print('XBIZ direct fallback failed:',exc);return []
    found=[];seen=set()
    for m in re.finditer(r'<a\b[^>]*href=["\']([^"\']*/news/\d+/[^"\']+)["\'][^>]*>(.*?)</a>',raw,re.I|re.S):
        href=html_lib.unescape(m.group(1));title=_strip_tags(m.group(2));low=title.lower()
        if len(title)<12 or any(t in low for t in EXPLICIT_SKIP) or not any(t in low for t in XBIZ_KEEP_HINTS):continue
        link=urljoin(url,href)
        if link in seen:continue
        published=_parse_xbiz_date(_strip_tags(raw[max(0,m.start()-700):min(len(raw),m.end()+900)]))
        if published is None:continue
        seen.add(link);found.append({'title':title,'link':link,'description':f'XBIZ reports an adult-entertainment industry development involving {title}.','pubDate':format_datetime(published),'published':published,'source':'XBIZ','category':'entertainment','imageUrl':'','_adult_source':True})
        if len(found)>=limit:break
    print(f'entertainment dirty/XBIZ direct: {len(found)} accepted');return found
def dedupe_rank(items,limit,mode):
    ranked=sorted(items,key=lambda x:(int(x.get('entertainmentScore') or 0),_published(x)),reverse=True);out=[];seen=set();source_counts={}
    for item in ranked:
        if item.get('entertainmentSafety')!=mode:continue
        k=_key(item);src=_source_key(item);cap=10 if src in ADULT_SOURCE_TOKENS else 5
...
def collect():
    candidates=[]
    query=core.QUERIES.get('entertainment',[]);combined=' OR '.join(f'({q})' for q in query) if isinstance(query,list) else query
    try:
        batch=core.parse_items(core.fetch(combined),'entertainment');candidates.extend(batch);print(f'entertainment primary: {len(batch)} accepted')
    except Exception as exc:print('Entertainment primary feed failed:',exc)
    for source,q in PREVIEW_SOURCES:
        try:
            batch=core.parse_items(core.fetch(q),'entertainment',source_override=source);candidates.extend(batch);print(f'entertainment clean-source/{source}: {len(batch)} accepted')
        except Exception as exc:print(f'Entertainment preview failed for {source}: {exc}')
    for source,q in DIRTY_PREVIEW_SOURCES:
        try:
            batch=core.parse_items(core.fetch(q),'entertainment',source_override=source);candidates.extend([{**x,'_forced_dirty':True} for x in batch]);print(f'entertainment dirty-source/{source}: {len(batch)} accepted')
        except Exception as exc:print(f'Entertainment dirty preview failed for {source}: {exc}')
    candidates.extend(fetch_xbiz_direct())

    def decorate_all(pool):
        signals=build_clean_signals(pool);return [d for item in pool if (d:=decorate(item,signals,pool,forced_dirty=bool(item.get('_forced_dirty')),adult_source=bool(item.get('_adult_source')) or _source_is_adult(item)))]

    decorated=decorate_all(candidates);clean=dedupe_rank(decorated,CLEAN_LIMIT,'clean')
    if len(clean)<CLEAN_MIN:
        print(f'Clean pool only {len(clean)}; running high-profile fallback queries')
        for q in CLEAN_FALLBACK_QUERIES:
            try:
                batch=core.parse_items(core.fetch(q),'entertainment');candidates.extend(batch);print(f'entertainment clean-fallback: {len(batch)} accepted')
            except Exception as exc:print('Entertainment Clean fallback failed:',exc)
        decorated=decorate_all(candidates);clean=dedupe_rank(decorated,CLEAN_LIMIT,'clean')
    ...
    if len(adult)<ADULT_MIN:
        extras=dedupe_rank([x for x in decorated if x.get('entertainmentSafety')=='dirty' and x.get('entertainmentLabel') in {'ADULT INDUSTRY','ADULT BUSINESS/LEGAL','CREATOR','ADULT AWARDS'}],ADULT_MIN,'dirty');keys={_key(x) for x in dirty}
        for x in extras:
            if _key(x) not in keys:dirty.append(x);keys.add(_key(x))
        dirty=sorted(dirty,key=lambda x:(int(x.get('entertainmentScore') or 0),_published(x)),reverse=True)[:DIRTY_LIMIT]
    if len(clean)<CLEAN_MIN:raise SystemExit(f'Clean Entertainment pool too small: {len(clean)} < {CLEAN_MIN}')
    if len(dirty)<DIRTY_MIN:raise SystemExit(f'Dirty Entertainment pool too small: {len(dirty)} < {DIRTY_MIN}')
    print('Selected Entertainment stories:',len(clean)+len(dirty));print('  Clean:',len(clean));print('  Dirty-only:',len(dirty));print('  Adult-industry:',sum(1 for x in dirty if x.get('entertainmentLabel') in {'ADULT INDUSTRY','ADULT BUSINESS/LEGAL','CREATOR','ADULT AWARDS'}))
    for idx,item in enumerate(clean[:10],1):print(f"  Clean #{idx}: score={item.get('entertainmentScore')} prominence={item.get('entertainmentProminence')} coverage={item.get('entertainmentCoverage')} {item.get('entertainmentLabel')} - {item.get('title')}")
...
def main():
    ...
    tree.write(NEWS,encoding='utf-8',xml_declaration=True);print(f'Wrote {len(selected)} Entertainment stories to News')
