...
OWNERSHIP_TABS=[x for x in CANONICAL_TABS if x not in RANKING_SURFACES|EDITORIAL_OVERLAYS]
...
"boxoffice":"Theatrical movie business: box-office performance, grosses, openings, ticket sales, theatrical releases and cinema industry performance. Not general celebrity entertainment."
}
...
"gaming":TabRule(D(video_game=13,gaming=11,playstation=12,xbox=12,nintendo=12,steam=9,game_studio=10,console=8,pc_gaming=11,esports=10,gameplay=8,dlc=7,game_release=9),D(casino=-20,gambling=-20,sportsbook=-20,lottery=-18,slot_machine=-18),9,11),
...
"military":TabRule(D(pentagon=14,armed_forces=13,troops=11,air_force=11,u_s_army=12,u_s_navy=12,marines=11,missile=10,airstrike=12,air_strike=12,drone_strike=12,warship=11,combat=10,battlefield=11,invasion=11,ceasefire=9,defense_department=14,centcom=14,military_operation=13,military_strike=13,military_deployment=12,munition=10,munitions=10,weapon_system=10,fighter_jet=10),D(border_war=-24,war_on_drugs=-22,price_war=-22,trade_war=-20,culture_war=-20,war_against=-18,veterans_museum=-18,veterans_day=-16,military_veteran=-10,war_memorial=-16),10,11),
...
"federal":TabRule(D(u_s_congress=13,congress=10,u_s_senate=12,house_of_representatives=12,u_s_supreme_court=14,scotus=14,department_of_justice=13,doj=11,fbi=11,dhs=11,u_s_treasury=11,treasury_department=11,epa=10,irs=10,federal_court=12,federal_judge=12,federal_agency=11,federal_government=10),D(federal_credit_union=-25,state_supreme_court=-25,state_senate=-22,state_house=-22,state_legislature=-22),10,9),
...
"nm":TabRule(D(new_mexico=15,santa_fe=10,albuquerque=10,las_cruces=10,rio_rancho=10,new_mexico_legislature=17,new_mexico_governor=16,nmdot=14),{},10,12),
...
"world":TabRule(D(ukraine=8,russia=8,china=7,iran=8,israel=8,gaza=8,france=7,germany=7,united_kingdom=7,japan=7,india=7,canada=7,mexico=7,afghanistan=8,saudi=8,taiwan=8,nato=9,united_nations=9,diplomacy=9,diplomatic=9,international=6,foreign_minister=8,prime_minister=7),{},7,5),
...
"boxoffice":TabRule(D(box_office=15,opening_weekend=13,weekend_gross=14,domestic_gross=14,worldwide_gross=13,theatrical=9,imax=9,ticket_sales=11,movie_release=8,film_release=8,cinema=6),D(movie_theater_shooting=-18,home_theater=-18,celebrity=-8,tv_series=-8,streaming_series=-8),10,11),
"x":TabRule({}, {},0,1),
"top":TabRule({}, {},0,0)
}
...
def obvious_noise(i):
    t=norm(clean_title(i))
    return any(x in t for x in [" obituary "," winning numbers "," lottery "," things to do "," classifieds "," job postings "," horoscope "," recipe "," prep roundup "," latest defense news weekly videos "," current & breaking news "," odds, picks and predictions ",".jpg "])
...
def raw(i,tab):
    ...
    if tab in {"nfl","presidential","military","technology","gaming","local","nm","region"} and title_strength==0:
        s=min(s,r.threshold-0.1)
    return round(s,2),ev

def score_tab_raw(i,tab):return raw(i,tab)[0]
def score_tab(i,tab):
    s,ev=raw(i,tab)
    if tab=="nm" and score_tab_raw(i,"local")>=RULES["local"].threshold:
        s-=10;ev.append("!local-specific")
    if tab=="region" and (score_tab_raw(i,"local")>=RULES["local"].threshold or score_tab_raw(i,"nm")>=RULES["nm"].threshold):
        s-=10;ev.append("!more-specific-geography")
    if tab in {"world","us"}:
        for x in ["military","technology","gaming","nfl","presidential","federal","legislation"]:
            if score_tab_raw(i,x)>=RULES[x].threshold+2:
                s-=7;ev.append("!specific:"+x)
    return round(s,2),ev
...
def best_tab(i,include_underreported=True):
    ...
    c=list(OWNERSHIP_TABS)
    ok=[t for t in c if scores[t]>=RULES[t].threshold]
    if not ok:return None,0.,scores
    ok.sort(key=lambda t:(RULES[t].specificity,scores[t]-RULES[t].threshold,scores[t]),reverse=True)
    w=ok[0]
    return w,scores[w],scores
...
def content_tokens(i):
    title,desc,_=parts(i)
    return {t for t in re.findall(r"[a-z0-9]+",title+desc) if len(t)>=3 and t not in STOPWORDS}
