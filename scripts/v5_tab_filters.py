...
RULES={
"nfl":TabRule({**D(nfl=12,national_football_league=14,super_bowl=12,nfl_draft=10,nfl_playoffs=10,nfl_season=9,football=4,quarterback=5),**NFL_FRANCHISE},D(college_football=-20,high_school_football=-20,ncaaf=-20,soccer=-14),9,10),
...
"military":TabRule(D(pentagon=12,armed_forces=11,troops=9,air_force=9,army=7,navy=7,marines=9,missile=8,airstrike=10,air_strike=10,strike=5,drone_strike=10,drone=6,warship=10,combat=8,battlefield=9,invasion=9,ceasefire=7,defense_department=12,defense=7,centcom=12,military_operation=11,military_strike=11,military=8,fighter_jet=8,fighter=6,military_aircraft=10,military_equipment=10,munition=8,munitions=8,warfighter=8,war=5),D(border_war=-20,war_on_drugs=-18,price_war=-18,trade_war=-16,culture_war=-16,war_against=-14,veterans_museum=-12),9,9),
"presidential":TabRule(D(president_trump=13,donald_trump=12,trump_administration=12,trump=7,white_house=11,executive_order=11,oval_office=11,press_secretary=8,presidential=8,commander_in_chief=9),D(former_president=-5,company_president=-16,university_president=-16,team_president=-16),9,9),
"legislation":TabRule(D(legislation=11,signed_into_law=13,lawmakers=6,statute=9,ordinance=10,final_rule=9,rulemaking=9,house_passed=9,senate_passed=9,veto=8,enacted=9,law_takes_effect=10,bill=6),D(lawsuit=-8,law_firm=-12,law_enforcement=-8),9,11),
"federal":TabRule(D(us_congress=11,congress=8,us_senate=10,senate=7,house_of_representatives=10,us_supreme_court=12,supreme_court=9,scotus=12,department_of_justice=11,doj=9,fbi=9,dhs=9,us_treasury=9,treasury_department=9,epa=8,irs=8,federal_court=10,federal_judge=10,federal_agency=9,federal=5),D(federal_credit_union=-20,state_supreme_court=-14),9,8),
...
"region":TabRule(D(arizona=8,colorado=8,utah=8,four_corners_region=11,flagstaff=8,phoenix=7,denver=7,salt_lake_city=7,southern_colorado=9,northern_arizona=9),D(california=-12,texas=-12,oregon=-12,washington_state=-12,new_hampshire=-20,college_football=-12,high_school_football=-12,basketball=-8),8,7),
"world":TabRule(D(ukraine=7,russia=7,china=6,iran=7,israel=7,gaza=7,france=6,germany=6,united_kingdom=6,japan=6,india=6,canada=6,mexico=6,afghanistan=7,saudi=7,taiwan=7,nato=8,united_nations=8,diplomacy=7,diplomatic=7,international=5),{},7,4),
"us":TabRule(D(us=8,united_states=8,america=6,american=5,americans=5,nationwide=7,across_the_country=7,national=3),{},7,3),
...
def obvious_noise(i):
    t=norm(clean_title(i));return any(x in t for x in [" obituary "," winning numbers "," lottery "," things to do "," classifieds "," job postings "," horoscope "," recipe "," prep roundup "," author "," latest defense news weekly videos "," current & breaking news "," odds, picks and predictions ",".jpg "])
...
def score_tab(i,tab):
    ...
    if tab in {"world","us"}:
        for x in ["military","technology","gaming","nfl","presidential","federal","legislation"]:
            if score_tab_raw(i,x)>=RULES[x].threshold:s-=7;ev.append("!specific:"+x)
    return round(s,2),ev
...
def best_tab(i,include_underreported=True):
    ...
    c=[t for t in OWNERSHIP_TABS if include_underreported or t!="underreported"];ok=[t for t in c if scores[t]>=RULES[t].threshold]
    if not ok:return None,0.,scores
    # Specific subject/geography tabs own an event before broad fallback tabs.
