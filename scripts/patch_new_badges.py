...
# Keep notification media state on window rather than in lexical `let` bindings.
# That avoids a browser temporal-dead-zone race when a user interaction lands while
# the generated script is still being initialized.
state = """let audioContext=null,audioUnlocked=false;
...
function primeNewArticleAudio(){if(window.__newArticleAudioPrimed&&audioUnlocked)return;void unlockPopAudio();}
...
print('NEW badges expire after one hour and notification audio uses stable window state.')
