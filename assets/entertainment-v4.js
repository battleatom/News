(function(){
  ...
  function currentMode(){return localStorage.getItem(MODE_KEY)==='clean'?'clean':'dirty'}

  function modeFiltered(items){
    const mode=currentMode();
    return items.filter(item=>{
      if(text(item,'category')!==KEY)return false;
      const safety=text(item,'entertainmentSafety')||'clean';
      return mode==='dirty'||safety!=='dirty';
    });
  }

  function renderEntertainment(items){
    const mode=currentMode();
    const filtered=modeFiltered(items);
    const data=typeof paginatedNewsItems==='function'?paginatedNewsItems(filtered):{available:filtered,visible:filtered,count:filtered.length};
    ...
    head.innerHTML=`<h2>${LABEL}</h2><button class="ent-mode-toggle ${mode}" type="button" title="${safe(modeTitle)}" aria-label="${safe(modeTitle)}">${actionLabel}</button><span class="count">Showing ${data.count} of ${data.available.length} stories</span>`;
    sec.appendChild(head);
    head.querySelector('.ent-mode-toggle')?.addEventListener('click',()=>{
