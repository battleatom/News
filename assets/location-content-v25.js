(function(){
  ...
  let NEWS_MARKETS=[];let MARKET_BY_ID={};let SOURCE_MARKET={};
  function normalizeSource(v){return String(v||'').toLowerCase().replace(/[^a-z0-9]+/g,' ').trim()}
  async function loadMarketDatabase(){try{const manifest=await fetch('data/us_news_markets.json?v=1',{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error(`market manifest ${r.status}`);return r.json()});const parts=await Promise.all((manifest.files||[]).map(path=>fetch(`data/${path}?v=1`,{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error(`market data ${r.status}`);return r.json()})));NEWS_MARKETS=parts.flatMap(x=>x.markets||[]);MARKET_BY_ID=Object.fromEntries(NEWS_MARKETS.map(m=>[m.id,m]));SOURCE_MARKET={};NEWS_MARKETS.forEach(m=>(m.sources||[]).forEach(src=>{SOURCE_MARKET[normalizeSource(src)]=m.id}));window.UnderreportedNewsMarkets={version:manifest.version||1,markets:NEWS_MARKETS,byId:MARKET_BY_ID,sourceMap:SOURCE_MARKET};refreshLocationView()}catch(err){console.warn('News market database unavailable; using feed metadata only.',err)}}
  ...
  function fallbackCounty(code,city){const c=String(city||'').toLowerCase();if(code==='NM'&&FARMINGTON_AREA.some(x=>c.includes(x)))return 'San Juan County';return ''}
  function location(){let cached=null;try{cached=window.UnderreportedLocation?.cached?.()||null}catch(e){}const code=String(cached?.state||localStorage.getItem('underreported-state')||'').replace(/^US-/,'').toUpperCase();const stateName=STATES[code]||'';const label=String(cached?.label||localStorage.getItem('underreported-location')||'');const city=String(cached?.city||label.split(',')[0]||'').trim();const county=String(cached?.county||localStorage.getItem('underreported-county')||fallbackCounty(code,city)||'').trim();const rawLat=cached?.lat,rawLon=cached?.lon;const lat=rawLat==null||rawLat===''?null:Number(rawLat),lon=rawLon==null||rawLon===''?null:Number(rawLon);return {code,name:stateName,city,county,region:STATE_REGION[code]||'',lat:Number.isFinite(lat)?lat:null,lon:Number.isFinite(lon)?lon:null,label}}
  ...
  function matchesExactCity(item,loc){if(!loc.city)return false;const city=loc.city.toLowerCase();if(text(item).includes(city))return true;const market=sourceMarket(item);return Boolean(market&&String(market.city||'').toLowerCase()===city&&(!loc.code||String(market.state||'').toUpperCase()===loc.code))}
  function matchesCity(item,loc){if(matchesExactCity(item,loc))return true;const t=text(item),city=String(loc.city||'').toLowerCase();if(loc.code==='NM'&&FARMINGTON_AREA.some(x=>city.includes(x)))return FARMINGTON_AREA.some(term=>t.includes(term));return false}
  function matchesCounty(item,loc){if(!loc.county)return false;const county=loc.county.toLowerCase();const t=text(item);if(t.includes(county))return true;if(loc.code==='NM'&&county==='san juan county')return FARMINGTON_AREA.some(term=>t.includes(term));return false}
  function matchesRegion(item,loc){return Boolean(loc.region&&itemRegion(item)===loc.region)}
  function sourceMarket(item){const explicit=field(item,['marketId']);if(explicit&&MARKET_BY_ID[explicit])return MARKET_BY_ID[explicit];const coords=itemCoords(item);if(explicit&&coords)return {id:explicit,city:field(item,['marketCity']),state:field(item,['marketState']),lat:coords.lat,lon:coords.lon,region:itemRegion(item)};const src=normalizeSource(field(item,['source']));const id=SOURCE_MARKET[src];return id?MARKET_BY_ID[id]:null}
  function marketGroups(items,loc){const groups=new Map();items.forEach(item=>{const market=sourceMarket(item);if(!market)return;if(!groups.has(market.id))groups.set(market.id,{market,items:[]});groups.get(market.id).items.push(item)});return [...groups.values()].map(g=>({...g,d:distanceMiles(loc,g.market)})).filter(g=>g.d!=null).sort((a,b)=>a.d-b.d)}
  function importance(item,loc){const t=text(item);let score=0;IMPACT.forEach(([term,value])=>{if(t.includes(term))score+=value});if(loc.city&&t.includes(loc.city.toLowerCase()))score+=18;if(loc.county&&t.includes(loc.county.toLowerCase()))score+=14;if(loc.name&&t.includes(loc.name.toLowerCase()))score+=9;const d=distanceToUser(item,loc);if(d!=null)score+=Math.max(0,30-d/8);const age=Math.max(0,(Date.now()-publishedMs(item))/3600000);score+=Math.max(0,18-age*.55);return score}
  function rank(items,loc){return unique(items).sort((a,b)=>importance(b,loc)-importance(a,loc)||publishedMs(b)-publishedMs(a))}
  function cloneAs(item,tier){const clone=item.cloneNode(true);let categoryEl=clone.querySelector('category');if(!categoryEl){categoryEl=clone.ownerDocument.createElement('category');clone.appendChild(categoryEl)}categoryEl.textContent='nm';let tierEl=clone.querySelector('locationTier');if(!tierEl){tierEl=clone.ownerDocument.createElement('locationTier');clone.appendChild(tierEl)}tierEl.textContent=tier;return clone}
  ...
  function countyPool(items){
    const loc=location();if(!loc.county)return [];
    const candidates=items.filter(i=>['local','region','nm','us','top'].includes(category(i))&&matchesCounty(i,loc));
    const countyWide=rank(candidates.filter(i=>!matchesExactCity(i,loc)),loc);
    const cityTail=rank(candidates.filter(i=>matchesExactCity(i,loc)),loc);
    return unique([...countyWide,...cityTail]).map(i=>cloneAs(i,loc.county)).slice(0,90);
  }
  function nearestLocalSelection(items,loc){
    const candidates=items.filter(i=>['local','region','nm','us','top'].includes(category(i)));const selected=[];const seen=new Set();const usedMarkets=[];
    const add=arr=>rank(arr,loc).forEach(item=>{const k=itemKey(item);if(k&&!seen.has(k)){seen.add(k);selected.push(item)}});
    add(candidates.filter(i=>matchesExactCity(i,loc)));
    for(const group of marketGroups(candidates.filter(i=>category(i)==='local'),loc)){
      ...
      if(group.d>LOCAL_RADIUS_MILES)break;
      add(group.items);usedMarkets.push(group.market.id);
    }
    if(selected.length<LOCAL_MIN_STORIES&&loc.lat!=null&&loc.lon!=null){const geotagged=candidates.map(item=>({item,d:distanceToUser(item,loc)})).filter(x=>x.d!=null&&x.d<=LOCAL_RADIUS_MILES).sort((a,b)=>a.d-b.d);add(geotagged.map(x=>x.item))}
    if(selected.length<LOCAL_MIN_STORIES)add(candidates.filter(i=>category(i)==='local'&&matchesCounty(i,loc)));
    return {selected,usedMarkets};
  }
  function localPool(items){const loc=location();if(!loc.city&&!loc.code&&loc.lat==null)return [];return nearestLocalSelection(items,loc).selected.map(i=>cloneAs(i,loc.city||'Local')).slice(0,90)}
  function regionPool(items){
    const loc=location();if(!loc.region)return [];
    const candidates=items.filter(i=>['region','local'].includes(category(i))&&matchesRegion(i,loc));
    const outsideState=[];const insideState=[];
    for(const item of candidates){(matchesState(item,loc)?insideState:outsideState).push(item)}
    const ordered=[...rank(outsideState,loc),...rank(insideState.filter(i=>!matchesCounty(i,loc)&&!matchesExactCity(i,loc)),loc),...rank(insideState.filter(i=>matchesCounty(i,loc)||matchesExactCity(i,loc)),loc)];
