...
from specialized import collect_nfl, collect_boxoffice

ROOT=Path(__file__).resolve().parents[1];WEB=ROOT/"web";DIST=ROOT/"dist"

def load_fixture(path: Path) -> list[Story]:
    return [Story(**row) for row in json.loads(path.read_text(encoding="utf-8"))]

def write_json(path: Path, payload) -> None:
    path.write_text(json.dumps(payload,indent=2,ensure_ascii=False)+"\n",encoding="utf-8")

def build(*,fixture: Path|None=None) -> dict:
    registry=load_registry();collected_errors=[]
    if fixture:
        raw=load_fixture(fixture);nfl=[];nfl_error="";boxoffice=[];boxoffice_error="TMDB not used in deterministic fixture build."
    else:
        raw,collected_errors=collect_all(registry);nfl,nfl_error=collect_nfl();boxoffice,boxoffice_error=collect_boxoffice()
    ...
    write_json(DIST/"boxoffice.json",{"generatedAt":generated,"movies":boxoffice,"error":boxoffice_error})
    status={"version":"6","generatedAt":generated,"storyCount":len(stories),"categoryCounts":{k:len(v) for k,v in by_category.items()},"collectorErrors":collected_errors,"nflError":nfl_error,"boxOfficeError":boxoffice_error,"buildMode":"fixture" if fixture else "live"}
