...
def near_duplicate(a: Story, b: Story) -> bool:
    if normalize_url(a.url) and normalize_url(a.url) == normalize_url(b.url):return True
    ta,tb=title_terms(a.title),title_terms(b.title)
    if not ta or not tb:return False
    smaller=min(len(ta),len(tb))
    if smaller>=5 and len(ta&tb)/smaller>=0.86:return True
...
def _summary_residual(story: Story) -> str:
    summary=normalize_title(story.summary);title=normalize_title(story.title);source=normalize_title(story.source);residual=summary
...
def technology_has_enough_information(story: Story) -> bool:
    ...
    title_words=[w for w in title.split() if len(w)>=3];residual_words=[w for w in residual.split() if len(w)>=3]
    if len(title_words)>=5 or len(title)>=42:return True
    return len(residual_words)>=8 and len(residual)>=55


def age_hours(story: Story, now: datetime) -> float:return max(0.0,(now-story.published_dt).total_seconds()/3600)

def presidential_has_enough_information(story: Story, now: datetime) -> bool:
    if age_hours(story,now)<=14*24:return True
    residual=_summary_residual(story);return len(residual)>=45 and len([w for w in residual.split() if len(w)>=3])>=7
...
def freshness_bucket(story: Story, now: datetime) -> int:
    """Keep genuinely fresh inventory ahead of older high-impact stories without discarding the latter."""
    age=age_hours(story,now)
    if age<=2:return 6
    if age<=6:return 5
    if age<=12:return 4
    if age<=24:return 3
...
def _source_key(value: str) -> str:return re.sub(r"[^a-z0-9]+","",(value or "").lower())
def _parse_dt(value: str) -> datetime:
    try:dt=datetime.fromisoformat((value or "").replace("Z","+00:00"))
    except Exception:return datetime.fromtimestamp(0,tz=timezone.utc)
...
def _corroboration_score(count:int)->int:return 15 if count<=0 else 40 if count==1 else 65 if count==2 else 80 if count==3 else 90 if count==4 else 100
def _freshness_score(dt:datetime,now:datetime)->int:
    ...
    return 0
def _saturation_penalty(count:int)->int:return 0 if count<=6 else 4 if count<=9 else 10 if count<=14 else 18
...
def _related_score(primary:Story,other:Story)->float:
    ...
    overlap=len(a&b)
    return 0.0 if overlap<2 else overlap/max(1,min(len(a),len(b)))
def _what_next(summary:str)->str:
    ...
    for sentence in re.split(r"(?<=[.!?])\s+",summary or ""):
        if len(sentence.strip())>=45 and any(cue in sentence.lower() for cue in cues):return sentence.strip()
    return ""
...
def _continuing_score(story:Story,now:datetime)->int:
    age_days=max(0.0,(now-story.published_dt).total_seconds()/86400);score=max(25,70-round(age_days*2.5));text=f"{story.title} {story.summary}".lower()
...
def enrich_underreported(visible_stories:list[Story],evidence_pool:list[Story],now:datetime)->None:
    for story in visible_stories:
        ...
        for _,_,other in candidates:
            ...
            seen_sources.add(skey);seen_urls.add(url);evidence.append({"title":other.title,"url":other.url,"source":other.source,"published_at":other.published_at})
            if len(evidence)>=24:break
        support_count=len(evidence);display=evidence[:4];story.supporting_source_count=support_count;story.coverage_gap_score=_coverage_gap_score(support_count);story.coverage_label=_coverage_label(support_count)
        ...
        if support_count==0:story.what_is_missing="No independent supporting publisher was found in the current approved coverage search. That is a coverage-gap signal, but the story is not treated as strongly corroborated until supporting reporting appears."
        elif support_count<=2:story.what_is_missing=f"Only {support_count} independent supporting publisher{'s were' if support_count!=1 else ' was'} found. The story remains lightly covered, but there is at least some independent corroboration."
        else:story.what_is_missing=f"The story has {support_count} independent supporting publishers. That corroboration verifies the event while the coverage-gap score measures how broadly it has spread beyond the primary report."
        ...
        if older:
            oldest=min(older,key=lambda r:_parse_dt(str(r.get("published_at",""))));oldest_dt=_parse_dt(str(oldest.get("published_at","")));age_days=max(1,(story.published_dt-oldest_dt).days)
        ...
        else:story.background="No older matched report was identified in the current evidence pool. This appears to be a newer development and the background section will deepen as related reporting is collected."
        story.what_next=_what_next(story.summary);recent6,recent24,prior72=_coverage_windows(evidence,story.source,now);story.recent_supporting_sources_6h=recent6;story.recent_supporting_sources_24h=recent24;story.prior_supporting_sources_72h=prior72;story.corroboration_score=_corroboration_score(support_count);story.freshness_score=_freshness_score(story.published_dt,now);story.coverage_momentum_score=_momentum_score(support_count,recent6,recent24,prior72,evidence);story.continuing_relevance_score=_continuing_score(story,now);story.saturation_penalty=_saturation_penalty(support_count);importance=_underreported_importance(story);priority=round(importance*.25+story.corroboration_score*.20+story.freshness_score*.20+story.coverage_gap_score*.15+story.coverage_momentum_score*.10+story.continuing_relevance_score*.10-story.saturation_penalty);story.underreported_priority=max(0,min(100,priority));story.coverage_score=story.underreported_priority;story.related=display
...
def process(stories:list[Story],registry:dict,*,now:datetime|None=None)->list[Story]:
    ...
    enrich_underreported(cleaned,evidence_pool,now);by_cat=defaultdict(list)
    ...
    for category,rows in by_cat.items():
        ...
        if category=="underreported":rows.sort(key=lambda s:(freshness_bucket(s,now),s.underreported_priority,s.freshness_score,s.coverage_momentum_score,s.importance,s.published_dt),reverse=True)
        else:rows.sort(key=lambda s:(freshness_bucket(s,now),s.importance,s.published_dt),reverse=True)
        if category=="x":
        ...
        else:chosen=select_diverse(rows,target,source_cap,near_duplicate)
        # Final order is recency-first inside the selected quality/diversity pool.
        chosen.sort(key=lambda s:(freshness_bucket(s,now),s.published_dt,s.importance),reverse=True)
        output.extend(chosen)
    category_order={name:i for i,name in enumerate(registry["categories"])};output.sort(key=lambda s:category_order[s.category]);return output