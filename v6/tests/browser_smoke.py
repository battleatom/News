...
def main():
    ...
    with sync_playwright() as p:
        ...
        dims=footer.evaluate("el=>{const s=getComputedStyle(el);return{position:s.position,width:el.getBoundingClientRect().width,card:el.closest('.story-card').getBoundingClientRect().width,gap:parseFloat(s.columnGap||s.gap),cols:s.gridTemplateColumns.split(' ').filter(Boolean).length}}");assert dims["position"]=="static";assert dims["width"]>=dims["card"]-40;assert dims["cols"]==3;assert dims["gap"]>=4
