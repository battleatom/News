...
def main():
    ...
    with sync_playwright() as p:
        ...
        footer=page.locator(".feedback-controls").first;assert footer.locator("button").count()==3;assert footer.get_by_text("D · Duplicate",exact=True).count()==1;assert footer.get_by_text("NR · Not Relevant",exact=True).count()==1;assert footer.get_by_text("NW · Not Wanted",exact=True).count()==1
        dims=footer.evaluate("el=>({position:getComputedStyle(el).position,width:el.getBoundingClientRect().width,card:el.closest('.story-card').getBoundingClientRect().width})");assert dims["position"]=="static";assert dims["width"]>=dims["card"]-8
