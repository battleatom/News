(()=>{
  ...
  document.addEventListener('click',async e=>{const b=e.target.closest('[data-full-health-sweep]');if(!b)return;if(b.disabled)return;const original=b.textContent;b.disabled=true;b.textContent='Sweeping all sources…';try{const r=await fetch('/api/admin/full-health-sweep',{method:'POST',cache:'no-store',headers:{'X-Underreported-Admin':'1'}}),d=await r.json();if(!r.ok||!d.ok)throw new Error(d.error||('HTTP '+r.status));b.textContent=`✓ ${d.healthy} healthy · ${d.failed} failed`;document.dispatchEvent(new CustomEvent('v6:fullhealthsweep',{detail:d}));await new Promise(resolve=>setTimeout(resolve,700));location.reload()}catch(err){b.textContent='Sweep failed';b.title=String(err?.message||err);setTimeout(()=>{b.textContent=original;b.disabled=false},3500)}},true);
  document.addEventListener('click',async e=>{const b=e.target.closest('[data-force-update]');if(!b)return;if(b.disabled)return;const original=b.textContent;b.disabled=true;b.textContent='Updating…';try{const [newsRes,serviceRes]=await Promise.all([fetch('/api/admin/force-update',{method:'POST',cache:'no-store',headers:{'X-Underreported-Admin':'1'}}),fetch('/api/admin/refresh-services',{method:'POST',cache:'no-store',headers:{'X-Underreported-Admin':'1'}})]),d=await newsRes.json(),services=await serviceRes.json();if(!newsRes.ok||!d.ok)throw new Error(d.error||('HTTP '+newsRes.status));b.textContent=`✓ ${d.newStories} new · ${d.recovered} recovered · ${d.failed} failed`;b.title=`${d.checked} sources checked · services ${services.ok?'refreshed':'partial'}`;document.dispatchEvent(new CustomEvent('v6:forceupdate',{detail:d}));await new Promise(resolve=>setTimeout(resolve,700));location.reload()}catch(err){b.textContent='Update failed';b.title=String(err?.message||err);setTimeout(()=>{b.textContent=original;b.disabled=false},3000)}},true);
